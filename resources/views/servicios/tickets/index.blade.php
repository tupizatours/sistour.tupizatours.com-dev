...
@section('content')
    ...
    <div class="card">
        <div class="card-header bg-primary pt-3 pb-3">
            <div class="d-flex align-items-center">
                <div>
                    <h6 class="mb-0 title_page text-white">LISTADO DE TICKETS</h6>
                </div>

                
                <div class="ms-auto">
                    <div class="d-flex">
                        <a href="{{ URL::to('service/tickets/eliminados') }}" class="btn btn-dark mt-2 mt-lg-0 font-13 btn">
                            ELIMINADOS
                        </a>
                    
                        <form action="{{ route('servtickets.store') }}" class="ms-1" method="POST" enctype="multipart/form-data">
                            @csrf

                            <button type="button" class="btn btn-primary mt-2 mt-lg-0 font-13 btn" data-bs-toggle="modal" data-bs-target="#ModalCreate">
                                <i class="bx bxs-plus-square"></i>NUEVO TICKET
                            </button>

                            @include('servicios.tickets.create')
                        </form>
                    </div>
                </div>
            </div>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="d-flex order-actions">
                                            <form action="{{ route('servtickets.update', $ticket->id) }}" class="" method="POST" enctype="multipart/form-data">
                                                @csrf
                                                @method('PUT')
        
                                                <button type="button" class="btn boton-eliminar ms-1" data-bs-toggle="modal" data-bs-target="#ModalEdit{{ $ticket->id }}">
                                                    <i class="bx bxs-edit"></i>
                                                </button>

                                                @include('servicios.tickets.edit')
                                            </form>
                                            ...
                                            <form action="{{ route('estatus.update', $ticket->id) }}" class="ms-1" method="POST">
                                                ...
                                                <button type="button" class="btn boton-eliminar ms-1" data-bs-toggle="modal" data-bs-target="#ModalPreDelete{{ $ticket->id }}">
                                                ...
                                                @include('servicios.tickets.predelete')
                                            </form>
                                        </div>
                                    </td>
                                </tr>
